...
import { Button, TextField, IconButton, Box, Table, TableBody, TableCell, TableHead, TableRow, Paper, Typography, Stack } from "@mui/material";
...
export default function LineItemsEditor({ lineItems, onChange }: any) {
  ...
  const totalLineItems = (lineItems || []).reduce((sum: number, item: any) => sum + (item.line_total || 0), 0);
  ...
  return (
    <Box>
      ...
      {(lineItems || []).length > 0 ? (
          ...
          <Table size="small">
            ...
            <TableBody>
              {(lineItems || []).map((li: any, idx: number) => (
                <TableRow key={li.id} sx={{ "&:hover": { backgroundColor: "#fafafa" } }}>
                  <TableCell>
                    <TextField
                  ...
                  <TableCell align="center">
                    <IconButton size="small" onClick={() => remove(idx)} color="error">
              ...
              {/* Total Row */}
              <TableRow sx={{ backgroundColor: "#f9f9f9", borderTop: "2px solid", borderColor: "divider" }}>
        ...
        <Paper variant="outlined" sx={{ p: 3, textAlign: "center", backgroundColor: "#fafafa" }}>
